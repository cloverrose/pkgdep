...
`pkgdep-tidy` is a tool to remove unused dependency rules from your configuration file.
...
```shell
$ go install github.com/cloverrose/pkgdep/cmd/pkgdep-tidy@latest
```
...
```bash
pkgdep-tidy -config=.pkgdep.yaml -inspector.file=used_rules.csv -output=.pkgdep.tidy.yaml
```
